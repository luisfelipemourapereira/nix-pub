...
let
  ...
  cfg = config.consul;
  cmd =
    if cfg.devMode then "@${cfg.bin} consul agent -dev"
    else
      "@${cfg.bin} consul agent -config-dir /etc/consul.d"
      + concatMapStrings (n: " -config-file ${n}") cfg.configFiles;
  ...
  options = {
    ...
    consul = {
      ...
      configFiles = mkOption {
        ...
        default = [ "/etc/consul.json" "/etc/consul-addrs.json" ]
    ...
    mkMerge [
      {
        ...
        environment = {
          etc."consul.json".text = builtins.toJSON cfg.configOptions;
